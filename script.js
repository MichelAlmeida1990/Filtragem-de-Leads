...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function processUploadedFile() {
        ...
        try {
            console.log('Processando arquivo:', currentFile.name);
            
            // Processar arquivo localmente usando XLSX.js (já carregado)
            ...
            console.log('Processamento local bem-sucedido:', result.rows, 'linhas');
            currentData = result;
            processDataForFiltering(result.original);
            showNotification(`Arquivo processado localmente! ${result.rows} linhas carregadas.`, 'success');
    ...
    function createSelectedLeadsTable() {
        ...
        // Campos específicos solicitados: Razão Social, CNPJ, Endereço, Telefone, Email
        ...
        const availableFields = importantFields.filter(field => headers.includes(field));
        
        // Se não encontrar os campos específicos, usar alguns alternativos
        if (availableFields.length === 0) {
            availableFields.push(...headers.slice(0, 6));
        }
        
        let tableHTML = `
            <div class="selected-table-wrapper">
                <table class="enhanced-selected-table">
                    <thead><tr>
                        <th class="col-index">#</th>
                        <th class="col-action">Ação</th>
        `;
        
        // Criar cabeçalhos com nomes amigáveis
        availableFields.forEach(field => {
            const friendlyName = getFieldFriendlyName(field);
            tableHTML += `<th class="col-data" title="${field}">${friendlyName}</th>`;
        });
        
        tableHTML += `</tr></thead><tbody>`;
        
        selectedLeads.forEach((lead, index) => {
            tableHTML += `<tr class="enhanced-selected-row" data-index="${index}">`;
            tableHTML += `<td class="col-index">${index + 1}</td>`;
            tableHTML += `<td class="col-action">
                            <button class="modern-remove-btn" onclick="removeSelectedLead(${index})" title="Remover lead">
            ...
            availableFields.forEach(field => {
                ...
                const displayValue = formattedValue.length > 40 ? formattedValue.substring(0, 40) + '...' : formattedValue;
                
                tableHTML += `<td class="col-data ${getFieldClass(field)}" title="${formattedValue}">
    ...
    // Função para nomes amigáveis dos campos
    function getFieldFriendlyName(field) {
        const fieldNames = {
            'NM_RAZAO_SOCIAL': '🏢 Razão Social',
            'NM_FANTASIA': '🏪 Nome Fantasia',
            'NR_CNPJ_CPF': '📄 CNPJ/CPF',
            'DC_END_FISICO_EC': '📍 Endereço',
            'BAIRRO': '🗺️ Bairro',
            'NM_MUNICIPIO': '🏙️ Cidade',
            'TEL1_CNPJ': '📞 Telefone 1',
            'TEL2_CNPJ': '📞 Telefone 2',
            'TEL1_CAD': '📱 Tel. Cadastro 1',
            'TEL2_CAD': '📱 Tel. Cadastro 2',
            'EMAIL': '📧 Email',
    ...
    // Função para formatar valores dos campos
    ...
    // Função para classes CSS específicas por campo
    function getFieldClass(field) {
        const fieldClasses = {
            'NM_RAZAO_SOCIAL': 'field-company',
            'NM_FANTASIA': 'field-name',
            'NR_CNPJ_CPF': 'field-document',
            'DC_END_FISICO_EC': 'field-address',
            'TEL1_CNPJ': 'field-phone',
            'TEL2_CNPJ': 'field-phone',
            'TEL1_CAD': 'field-phone',
            'TEL2_CAD': 'field-phone',
            'EMAIL': 'field-email',
            'EMAIL_CONTATO': 'field-email'
        };
        return fieldClasses[field] || 'field-default';
    }
