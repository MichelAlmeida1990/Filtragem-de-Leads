document.addEventListener('DOMContentLoaded', function() {
    ...
    function populateFilters() {
        ...
        if (cityColumn) {
            const cities = [...new Set(originalData.map(row => row[cityColumn]).filter(city => city))];
        ...
        if (areaColumn) {
            const areas = [...new Set(originalData.map(row => row[areaColumn]).filter(area => area))];
    ...
    function displayResults() {
        ...
        originalContent.textContent = formatDataForDisplay(originalData);
        
        // Display filtered data
        filteredContent.textContent = formatDataForDisplay(filteredData);
