document.addEventListener('DOMContentLoaded', function() {
    ...
    function switchOption(option) {
        ...
        if (!document.getElementById('filters-section').classList.contains('active')) {
            activateIcon2(); // Ativar seção de filtros
        }
...
function updateDashboardStats() {
    ...
    const totalCities = originalData ? new Set(originalData.map(row => row['NM_MUNICIPIO'] || row['CIDADE'] || '')).size : 0;
    const totalAreas = originalData ? new Set(originalData.map(row => row['NM_GEO'] || row['AREA'] || '')).size : 0;
...
window.exportToExcel = function() {
    ...
    try {
        // Preparar dados
        const worksheetData = [headers, ...filteredData.map(row => headers.map(header => row[header] || ''))];
...
window.exportToPdf = function() {
    ...
    try {
        ...
        const tableData = filteredData.slice(0, 100).map(row => 
            headers.map(header => String(row[header] || '').substring(0, 20))
        );
        
        // Adicionar tabela
        doc.autoTable({
            head: [headers.map(h => h.substring(0, 15))],
            body: tableData,
            startY: 40,
            styles: {
                fontSize: 8,
