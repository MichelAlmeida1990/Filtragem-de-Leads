...
function toggleTetris() {
    ...
    if (tetrisEnabled) {
        // Desligar Tetris
        if (window.cieloTetris) {
            window.cieloTetris.destroy();
            window.cieloTetris = null;
        }
        tetrisEnabled = false;
        btn.textContent = '🎮 Tetris OFF';
        btn.classList.add('off');
        showNotification('Tetris desativado! 🎮', 'info');
    } else {
        // Ligar Tetris
        if (window.startCieloTetris) {
            window.startCieloTetris();
        }
        tetrisEnabled = true;
        btn.textContent = '🎮 Tetris ON';
        btn.classList.remove('off');
        showNotification('Tetris ativado! Use as setas para jogar! 🎮', 'success');
    }
}