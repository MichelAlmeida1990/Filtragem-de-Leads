document.addEventListener('DOMContentLoaded', function() {
    ...
    function populateFilters() {
        ...
        if (cityColumn) {
            const cities = [...new Set(originalData.map(row => row[cityColumn]).filter(city => {
                // Filtrar apenas valores que são realmente cidades
                if (!city || city.trim() === '') return false;
                
                const cityValue = city.toString().trim();
                
                // Excluir valores que são números ou códigos
                if (/^\d+$/.test(cityValue)) return false; // Apenas números
                if (/^\d{2}$/.test(cityValue)) return false; // Códigos de 2 dígitos
                if (/^\d{3}$/.test(cityValue)) return false; // Códigos de 3 dígitos
                if (/^\d{4}$/.test(cityValue)) return false; // Códigos de 4 dígitos
                if (/^\d{5}$/.test(cityValue)) return false; // Códigos de 5 dígitos
                if (/^\d{6}$/.test(cityValue)) return false; // Códigos de 6 dígitos
                if (/^\d{7}$/.test(cityValue)) return false; // Códigos de 7 dígitos
                if (/^\d{8}$/.test(cityValue)) return false; // Códigos de 8 dígitos
                if (/^\d{9}$/.test(cityValue)) return false; // Códigos de 9 dígitos
                if (/^\d{10}$/.test(cityValue)) return false; // Códigos de 10 dígitos
                if (/^\d{11}$/.test(cityValue)) return false; // Códigos de 11 dígitos
                if (/^\d{12}$/.test(cityValue)) return false; // Códigos de 12 dígitos
                if (/^\d{13}$/.test(cityValue)) return false; // Códigos de 13 dígitos
                if (/^\d{14}$/.test(cityValue)) return false; // Códigos de 14 dígitos
                
                // Excluir valores que são códigos com letras e números
                if (/^[A-Z]{2}\s*-\s*[A-Z]+/.test(cityValue)) return false; // Padrão "XX - NOME"
                if (/^CN\s*-\s*[A-Z]+/.test(cityValue)) return false; // Padrão "CN - NOME"
                if (/^ROTA\s*\d+/.test(cityValue)) return false; // Padrão "ROTA X"
                if (/^SEMANA/.test(cityValue)) return false; // Valores que começam com "SEMANA"
                if (/^INATIVO/.test(cityValue)) return false; // Valores que começam com "INATIVO"
                if (/^FRANQUIAS/.test(cityValue)) return false; // Valores que começam com "FRANQUIAS"
                
                // Excluir valores muito curtos (provavelmente códigos)
                if (cityValue.length <= 2) return false;
                
                // Excluir valores que são apenas números e letras misturados sem espaços
                if (/^[A-Z0-9]+$/.test(cityValue) && cityValue.length <= 10) return false;
                
                // Incluir apenas valores que parecem nomes de cidades
                // Deve ter pelo menos 3 caracteres e conter letras
                return cityValue.length >= 3 && /[A-Za-z]/.test(cityValue);
            }))];
        ...
        if (areaColumn) {
            const areas = [...new Set(originalData.map(row => row[areaColumn]).filter(area => {
                // Filtrar apenas valores que são realmente áreas
