...
class CieloBackgroundAnimation {
    ...
    init() {
        console.log('🎯 Inicializando CieloBackgroundAnimation...');
        this.createCanvas();
        console.log('📐 Canvas criado');
        this.createPieces();
        console.log('🧩 Peças criadas:', this.pieces.length);
        this.createParticles();
        console.log('✨ Partículas criadas:', this.particles.length);
        this.createBanners();
    ...
    createCanvas() {
        ...
        try {
            ...
            console.log('📝 Canvas criado, adicionando ao body...');
            ...
            // Verificar se canvas foi realmente adicionado
            const addedCanvas = document.getElementById('cieloBackground');
            if (addedCanvas) {
                console.log('✅ Canvas adicionado com sucesso ao DOM');
            } else {
                throw new Error('Canvas não foi adicionado ao DOM');
            }
    ...
    createPieces() {
        this.pieces = [];
        const numPieces = Math.floor(window.innerWidth / 80); // MÁXIMO DE PEÇAS
        ...
        for (let i = 0; i < numPieces; i++) {
            ...
            this.pieces.push({
                ...
                vx: (Math.random() - 0.5) * 2.0, // Velocidade MÁXIMA
                vy: Math.random() * 1.8 + 0.6, // Velocidade vertical MÁXIMA
                rotation: Math.random() * Math.PI * 2,
                rotationSpeed: (Math.random() - 0.5) * 0.1, // Rotação SUPER RÁPIDA
                scale: Math.random() * 1.2 + 0.8, // Escala MÁXIMA
                shape: shape,
                color: pieceType.color,
                emoji: pieceType.emoji,
                name: pieceType.name,
                opacity: Math.random() * 0.4 + 0.8, // Opacidade ULTRA ALTA
    ...
    createParticles() {
        this.particles = [];
        const numParticles = 200; // MÁXIMO DE PARTÍCULAS
        ...
        for (let i = 0; i < numParticles; i++) {
            this.particles.push({
                ...
                size: Math.random() * 8 + 5, // Partículas SUPER GRANDES
                color: this.colors.azulVivido,
                opacity: Math.random() * 0.5 + 0.7, // Opacidade MÁXIMA
    ...
    drawPieceShape(piece) {
        ...
        // Emoji no centro SUPER GRANDE
        this.ctx.font = `${blockSize * 1.5}px Arial`;
    ...
    animate() {
        try {
            ...
            // Log inicial só uma vez
            if (this.time < 100) {
                console.log('🎬 Loop de animação funcionando!');
            }
...
function startCieloBackground() {
    console.log('🚀 startCieloBackground chamada');
    try {
        if (cieloBackground) {
            console.log('🗑️ Destruindo animação existente');
...
function stopCieloBackground() {
    console.log('🛑 stopCieloBackground chamada');
    try {
        if (cieloBackground) {
            cieloBackground.destroy();
            cieloBackground = null;
            console.log('✅ Animação parada com sucesso');
        } else {
            console.log('⚠️ Nenhuma animação para parar');
        }
    } catch (error) {
        console.error('❌ Erro ao parar animação:', error);
    }
}

// Sistema de diagnóstico completo
function diagnosticarAnimacoes() {
    console.log('🔍 === DIAGNÓSTICO DE ANIMAÇÕES ===');
    console.log('📄 DOM ready:', document.readyState);
    console.log('🌐 Window loaded:', document.readyState === 'complete');
    console.log('📐 Canvas existe:', !!document.getElementById('cieloBackground'));
    console.log('🎨 Instância existe:', !!cieloBackground);
    console.log('🔧 Funções globais:', {
        startCieloBackground: typeof window.startCieloBackground,
        stopCieloBackground: typeof window.stopCieloBackground
    });
    
    // Tentar forçar criação
    if (!document.getElementById('cieloBackground')) {
        console.log('🚀 Forçando criação das animações...');
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🎨 DOMContentLoaded - Iniciando animações Cielo...');
    setTimeout(() => {
        diagnosticarAnimacoes();
        startCieloBackground();
        console.log('✨ Primeira tentativa de inicialização concluída!');
    }, 100);
});

// Segunda tentativa após window load
window.addEventListener('load', () => {
    console.log('🌐 Window load - Verificando animações...');
    setTimeout(() => {
        diagnosticarAnimacoes();
        if (!document.getElementById('cieloBackground')) {
...
setTimeout(() => {
    console.log('⏰ Verificação final das animações...');
    diagnosticarAnimacoes();
    if (!document.getElementById('cieloBackground')) {
...
window.testarAnimacoes = function() {
    console.log('🧪 === TESTE MANUAL DE ANIMAÇÕES ===');
    diagnosticarAnimacoes();
    
    // Forçar criação
    console.log('🔧 Forçando nova animação...');
    startCieloBackground();
    
    // Verificar após 1 segundo
    setTimeout(() => {
        const canvas = document.getElementById('cieloBackground');
        if (canvas) {
            console.log('✅ SUCESSO! Canvas criado:', {
                width: canvas.width,
                height: canvas.height,
                visible: canvas.style.opacity,
                zIndex: canvas.style.zIndex
            });
        } else {
            console.log('❌ FALHA! Canvas não foi criado');
        }
    }, 1000);
};
...
window.diagnosticarAnimacoes = diagnosticarAnimacoes;

// Log final
console.log('🎯 tetris.js carregado! Use window.testarAnimacoes() para testar manualmente.');